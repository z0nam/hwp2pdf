...
## 3-1. 서버가 불가할 때: rhwp 대체 변환 (선택)

서버에 연결할 수 없을 때 **로컬에서 rhwp로 PDF만** 만들어 두는 선택지가 있습니다.
기본은 꺼져 있고, 옵션에서 `변환 서버에 연결할 수 없으면 rhwp로 대체`를 켜야 동작합니다.

### 설치

[rhwp](https://github.com/edwardkim/rhwp)는 Rust로 만든 HWP/HWPX 렌더 엔진(MIT)입니다.
플랫폼별 릴리스 바이너리로 배포되므로 소스를 받아 빌드할 필요가 없고, **레포에는 포함하지
않습니다**(플랫폼 종속·10MB). 받아오는 스크립트가 있습니다:

```bash
./scripts/fetch_rhwp.sh              # macOS / Linux
.\scripts\fetch_rhwp.ps1            # Windows
```

`curl`(또는 `Invoke-WebRequest`)만 쓰므로 **GitHub 계정도 `gh` CLI도 필요 없습니다.**
릴리스의 `SHA256SUMS.txt`로 체크섬을 검증한 뒤 `vendor/rhwp/`에 설치하고, hwp2pdf가
자동으로 찾습니다. 버전은 스크립트 상단에 핀돼 있습니다(현재 `v0.8.4`).

직접 받아 두셨다면 경로를 알려주면 됩니다 — 탐색 순서는
`--rhwp-path` → `$HWP2PDF_RHWP` → `PATH` → `vendor/rhwp/` 입니다.

```bash
hwp2pdf ~/문서/보고서폴더 --pdf --rhwp-fallback
```

### ⚠️ rhwp는 한컴 엔진이 아닙니다

별도로 구현된 렌더러라 **결과가 원본과 다릅니다.** 저장소의 390쪽 보고서 픽스처로
한컴 네이티브와 비교한 실측(rhwp v0.8.4):

| 항목 | 한컴 네이티브 | rhwp v0.8.4 |
|---|---|---|
| 소요 시간 | 76초 | 48초 (더 빠름) |
| 파일 크기 | 2.9MB | 30.9MB (약 10배) |
| 전체 쪽수 | 379쪽 | 379쪽 — 일치 |
| 머리말 쪽번호 | `차 례 11` | `차 례 9` — **번호가 다름** |
| 목차 리더 뒤 쪽번호 | `……… 327` | 점선만, **숫자 없음** |
| 대시 글리프 | `− 우근민` | `?우근민` — **깨짐** |
| 글머리표 | `・` | `.` |

v0.7.17에서는 쪽수까지 390쪽으로 어긋났고 머리말 쪽번호가 아예 없었습니다. v0.8.4에서
그 둘은 해결됐지만 위 차이는 남아 있습니다. 표지처럼 단순한 쪽은 육안으로 거의
같아 보이므로 더 주의해야 합니다.

폰트 문제가 아닙니다 — 시스템에 한컴 폰트가 있어도 동일하고, `export-pdf`에는
`--font-path` 옵션 자체가 없습니다.

**그래서 이렇게 쓰세요:**

- ✅ 서버가 죽었는데 **지금 내용을 봐야 할 때**
- ✅ 대략적인 레이아웃 확인
- ❌ **남에게 보낼 최종본** — 서버 복구 후 다시 변환하세요

**DOCX는 만들 수 없습니다.** rhwp는 PDF 전용이라 DOCX 작업은 실패로 기록됩니다.

대체 변환이 쓰이면 숨기지 않습니다. 로그에 서버 실패 이유와 품질 경고가 찍히고,
성공 줄이 `성공 PDF (rhwp)`로 남아 나중에도 어느 엔진 산출물인지 구분됩니다.

## 4. 클라이언트 설정 (mac)

### 주소를 알아내는 세 가지 방법

처음 설치하는 사람이 가장 먼저 막히는 지점입니다. 위의 연결 방법 3가지와 짝을 이룹니다.

| 상황 | 방법 |
|---|---|
| 서버 담당자와 연락이 닿는다 | **초대 문자열**을 받아 주소칸에 붙여넣기 (가장 확실) |
| 옆자리 Windows PC의 **이름**은 안다 | 이름만 입력 (`namun-ji`) |
| Tailscale이나 같은 네트워크에 있다 | **찾기** 버튼 |

#### 초대 문자열

서버는 시작할 때 로그에 한 줄을 찍습니다. 주소와 토큰이 같이 들어 있어
전화로 불러줄 것이 없습니다.

```
  invite      hwp2pdf://eyJ1IjoiaHR0cDovLzEwMC4xMjQuMTE3Ljc1OjE3NjUwIiwidCI6Ii4uLiJ9
              paste this into the client's address field
```

로그 위치는 콘솔 빌드면 화면, 창 없는 빌드면
`%LOCALAPPDATA%\hwp2pdf\server.log`입니다. 이 한 줄을 클라이언트의 **주소**
칸에 붙여넣으면 주소와 토큰이 동시에 채워집니다. 서브넷·VPN·포트포워딩을
가리지 않으므로, 아래 두 방법이 막히는 회사망에서도 동작합니다.

`--bind`가 루프백이면 다른 기기에서 쓸 수 없는 주소라 초대 문자열 대신 경고가
찍힙니다. `--bind tailscale` 또는 `--bind <주소>`로 다시 띄우세요.

#### 이름·IP만 입력

주소칸은 아래를 모두 받아 `http://호스트:17650`으로 보정합니다.

| 입력 | 해석 |
|---|---|
| `namun-ji` | `http://namun-ji:17650` (`namun-ji.local`도 자동 재시도) |
| `192.168.0.5` | `http://192.168.0.5:17650` |
| `namun-ji:9000` | `http://namun-ji:9000` |
| `https://convert.example.com` | 그대로 (스킴을 직접 썼으면 포트를 넣지 않습니다) |

Windows 10 이상은 자기 호스트 이름을 mDNS로 알리므로, mac에서는 보통
`이름.local`이 DNS 서버 없이도 풀립니다. 그래서 PC 이름만 알아도 충분합니다.

#### 찾기 버튼

주소칸 옆 **찾기**를 누르면 이 컴퓨터가 이미 볼 수 있는 기기에
`/v1/health`를 던져 응답한 서버를 목록으로 보여줍니다. 후보는 두 곳에서 옵니다.

- **Tailscale 피어** — `tailscale status --json`의 온라인 노드. 서브넷을 넘어갑니다.
- **ARP 이웃** — 이 맥이 최근 통신한 같은 네트워크의 호스트. OS가 이미 들고 있는
  표를 읽을 뿐이라 포트 스캔이 아니고, 대신 **한동안 조용했던 서버는 놓칩니다.**

둘 다 최선 노력(best-effort)입니다. Tailscale이 없거나 ARP 표가 비어 있으면
조용히 빈 목록이 되고, 그때는 초대 문자열이나 직접 입력으로 갑니다.
`/v1/health`는 토큰 없이 응답하도록 설계돼 있어 **토큰이 걸린 서버도 목록에는
뜹니다** — 목록에서 고른 뒤 토큰만 채우면 됩니다.

#### 더 넓게 찾기 (대역 훑기)

찾기 결과가 비었을 때만 **더 넓게 찾기** 버튼이 나타납니다. 누르면 이 컴퓨터가
붙어 있는 사설 대역(`10/8`, `172.16/12`, `192.168/16`)의 `/24`를 훑어
17650 포트를 직접 확인합니다.

- **첫 클릭으로는 절대 돌지 않습니다.** 대역당 254개 연결은 회사망 보안 장비에
  포트 스캔으로 보일 수 있어, 사용자가 한 번 더 명시적으로 눌러야 시작합니다.
- 기본 경로 하나가 아니라 **붙어 있는 모든 인터페이스**의 대역을 봅니다. 유선과
  무선을 동시에 쓰는 맥은 서버가 기본 경로 쪽에 없는 경우가 흔합니다
  (최대 3개 대역). Tailscale 대역(`100.64/10`)·링크로컬·루프백은 제외합니다.
- 실측: 2개 대역 506개 주소를 약 5초.

CLI에서도 같은 탐색을 씁니다.

```bash
hwp2pdf find            # Tailscale 피어 + ARP 이웃
hwp2pdf find --wide     # 위에 더해 붙어 있는 대역 전체
hwp2pdf find --json     # 스크립트용
```
...
`--server`는 GUI 주소칸과 같은 것을 받습니다 — 전체 URL, 이름만, `호스트:포트`,
그리고 초대 문자열(이 경우 토큰까지 같이 들어옵니다).

```bash
hwp2pdf ~/문서/보고서폴더 --pdf --server namun-ji --token <토큰>
hwp2pdf ~/문서/보고서폴더 --pdf --server 'hwp2pdf://eyJ1Ijoi...'
```
...
| `변환 서버와 프로토콜 버전이 다릅니다` | 프로토콜(`API_VERSION`)이 바뀐 릴리스입니다. 서버와 클라이언트를 함께 갱신하세요. **빌드 버전이 달라도 프로토콜이 같으면 정상 동작하므로**, 이 메시지가 안 뜨면 굳이 맞출 필요 없습니다 |
