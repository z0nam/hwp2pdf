...
export HWP2PDF_RHWP=/path/to/rhwp     # 또는 --rhwp-path, 또는 PATH에 두기
```

### ⚠️ rhwp는 한컴 엔진이 아닙니다

rhwp는 별도로 구현된 HWP 렌더러입니다. 빠르고 서버 없이 돌지만 **결과가 원본과 다릅니다.**
저장소의 390쪽 보고서 픽스처로 한컴 네이티브 변환과 비교한 실측:

| 항목 | 한컴 네이티브 | rhwp |
|---|---|---|
| 소요 시간 | 76초 | 47초 (더 빠름) |
| 파일 크기 | 3.1MB | 31.7MB (약 10배) |
| **전체 쪽수** | **379쪽** | **390쪽** |
| 머리말 쪽번호 | `차 례 11` | `차 례` — **누락** |
| 목차 쪽번호 | `……… 327` | 점선만, **숫자 없음** |
| 대시 글리프 | `− 우근민` | `?우근민` — **깨짐** |

표지처럼 단순한 쪽은 거의 같아 보이지만, 본문에서는 위와 같이 갈립니다.
폰트 문제가 아닙니다(시스템에 한컴 폰트가 있어도 동일하며, `export-pdf`에는
`--font-path` 옵션 자체가 없습니다). 쪽번호는 필드 렌더링 미구현입니다.
