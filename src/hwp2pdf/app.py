...
APP_TITLE = "HWP/HWPX/DOCX -> PDF Converter"
...
def enabled_extensions(include_docx: bool):
...
class ConverterApp:
    def __init__(self, root: tk.Tk):
        ...
        self.include_docx_var = tk.BooleanVar(value=False)
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(opts, text="Overwrite existing PDF", variable=self.overwrite_var).grid(
            row=0, column=1, sticky="w", padx=(0, 16)
        )
        ttk.Checkbutton(opts, text="Also convert DOCX files", variable=self.include_docx_var).grid(
        ...
        notes = (
            ...
            "- DOCX conversion uses Hancom Office COM import/export, so layout fidelity depends on Hancom's DOCX support.\n"
    ...
    @staticmethod
    def collect_files(folder: str, recursive: bool, include_docx: bool):
        root = Path(folder)
        iterator = root.rglob("*") if recursive else root.glob("*")
        allowed_extensions = enabled_extensions(include_docx)
        files = []
    ...
    def _run_conversion(self, folder: str, recursive: bool, overwrite: bool, use_safe_copy: bool, include_docx: bool):
        try:
            files = self.collect_files(folder, recursive, include_docx)
            total = len(files)
            extension_label = ", ".join(ext.upper() for ext in enabled_extensions(include_docx))
            if total == 0:
            ...
            try:
                ...
                self.log_queue.put(("log", f"Found {total} file(s): {extension_label}"))
                self.log_queue.put(("log", f"CSV log: {log_csv}"))
                self.log_queue.put(("log", f"Safe temp mode: {'ON' if use_safe_copy else 'OFF'}"))
                self.log_queue.put(("log", f"DOCX conversion: {'ON' if include_docx else 'OFF'}"))
                ...
                with open(log_csv, "w", newline="", encoding="utf-8-sig") as f:
                    ...
                    for idx, src_path in enumerate(files, start=1):
                        ...
                        try:
                            ...
                            if opened is False:
                                raise RuntimeError(f"Open failed as {fmt}")
