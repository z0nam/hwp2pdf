...
TEXT = {
    "ko": {
        ...
        "remote_version_mismatch": "변환 서버와 프로토콜 버전이 다릅니다 — 서버 API {api}, 이 앱 API {client_api} (서버 빌드 {server}).\n\n프로토콜이 바뀐 릴리스이므로 서버와 클라이언트를 함께 갱신해야 합니다.",
        ...
        "rhwp_fallback_option": "변환 서버에 연결할 수 없으면 rhwp로 대체 (PDF만, 근사 품질)",
        "rhwp_not_found": "rhwp 실행 파일을 찾지 못했습니다.\n\n경로를 설정하거나 환경변수 {var}에 지정하세요.",
        "rhwp_engaged": "rhwp로 대체 변환합니다: {path}",
        "rhwp_quality_note": "rhwp는 한컴 엔진이 아니라 별도 렌더러입니다. 쪽 나눔·머리말 쪽번호·일부 글리프가 원본과 다를 수 있으니, 배포용 최종본은 서버 복구 후 다시 변환하세요.",
        "rhwp_format_unsupported": "rhwp는 PDF만 만들 수 있습니다. {format}은(는) 변환 서버가 필요합니다.",
        "rhwp_failed": "rhwp 변환 실패: {detail}",
        "rhwp_timeout": "rhwp 변환이 {seconds}초 안에 끝나지 않았습니다.",
        "fallback_engaged": "변환 서버를 쓸 수 없어 대체 엔진으로 진행합니다 — {detail}",
    ...
    "en": {
        ...
        "remote_version_mismatch": "Protocol version differs from the conversion server -- server API {api}, this app API {client_api} (server build {server}).\n\nThis release changed the protocol, so the server and the client have to be updated together.",
        ...
        "rhwp_fallback_option": "Fall back to rhwp when the conversion server is unreachable (PDF only, approximate)",
        "rhwp_not_found": "Could not find the rhwp executable.\n\nSet its path in the settings, or point {var} at it.",
        "rhwp_engaged": "Converting with rhwp instead: {path}",
        "rhwp_quality_note": "rhwp is an independent renderer, not Hancom's engine. Pagination, header page numbers and some glyphs can differ from the original, so re-convert anything you intend to send once the server is back.",
        "rhwp_format_unsupported": "rhwp can only produce PDF. {format} needs the conversion server.",
        "rhwp_failed": "rhwp conversion failed: {detail}",
        "rhwp_timeout": "rhwp did not finish within {seconds}s.",
        "fallback_engaged": "The conversion server is unavailable, so a fallback engine is being used -- {detail}",
