...
TEXT = {
    "ko": {
        ...
        "pick_file": "파일 선택...",
        "options": "옵션",
        "engine_status_title": "변환 엔진 상태",
        "hancom_install_checking": "한컴오피스 한글: 확인 중...",
        "hancom_install_ready": "한컴오피스 한글: 설치됨",
        "hancom_install_missing": "한컴오피스 한글: 설치되지 않음",
        "hancom_install_unsupported": "로컬 한컴오피스 한글: 사용 불가 (macOS)",
        "hwp_status_running": "아래한글 실행 상태: 실행 중",
        "hwp_status_not_running": "아래한글 실행 상태: 실행 중 아님",
        ...
        "auto_update_confirm_macos": "v{latest}로 자동 업데이트할까요?\n\n다운로드 후 앱을 새 버전으로 교체하고 hwp2pdf를 다시 엽니다.",
        "auto_update_portable": "자동 업데이트는 설치본에서만 지원됩니다. 브라우저에서 직접 다운로드할까요?",
        "auto_update_portable_macos": "이 앱은 스스로 교체할 수 없는 위치에 있습니다. hwp2pdf.app을 응용 프로그램 폴더처럼 쓰기 가능한 곳으로 옮기면 자동 업데이트가 동작합니다.\n\n브라우저에서 직접 받을까요?",
        ...
        "update_status_critical": "\u26a0 보안/중요 업데이트가 있습니다. 현재: {current} / 최신: {latest}",
        "update_critical_prompt": "이 업데이트(v{latest})는 즉시 적용을 권장합니다.\n\n지금 업데이트할까요?",
        ...
        "select_output": "출력 형식을 하나 이상 선택하세요: PDF, DOCX 또는 HWPX.",
        "pywin32_missing": "pywin32를 사용할 수 없습니다.\n\n설치 명령:\npython -m pip install pywin32\n\n상세:\n{detail}",
        "local_hwp_missing": "한컴오피스 한글을 찾지 못했습니다.\n\n한컴오피스를 설치하거나 COM 등록을 확인하세요.\n\n상세: {detail}",
        "local_hwp_start_failed": "한컴오피스 한글을 시작하지 못했습니다.\n\n상세: {detail}",
        ...
        "rhwp_fallback_option": "한컴 변환을 사용할 수 없을 때 rhwp로 PDF 대체",
        "rhwp_fallback_help_local": "한컴오피스가 없거나 시작할 수 없을 때 적용 · PDF만 가능 · 결과가 원본과 다를 수 있음",
        "rhwp_fallback_help_remote": "변환 서버가 없거나 연결할 수 없을 때 적용 · PDF만 가능 · 결과가 원본과 다를 수 있음",
        "rhwp_status_ready": "rhwp 설치 상태: 설치됨",
        "rhwp_status_missing": "rhwp 설치 상태: 설치되지 않음",
        "rhwp_not_found": "rhwp 실행 파일을 찾지 못했습니다.\n\n경로를 설정하거나 환경변수 {var}에 지정하세요.",
        "rhwp_engaged": "rhwp로 대체 변환합니다: {path}",
        "rhwp_quality_note": "rhwp는 한컴 엔진이 아니라 별도 렌더러입니다. 쪽 나눔·머리말 쪽번호·일부 글리프가 원본과 다를 수 있으니, 배포용 최종본은 한컴 변환이 가능해진 뒤 다시 변환하세요.",
        "rhwp_format_unsupported": "rhwp는 PDF만 만들 수 있습니다. {format}은(는) 한컴 변환 엔진이 필요합니다.",
        "rhwp_failed": "rhwp 변환 실패: {detail}",
        "rhwp_timeout": "rhwp 변환이 {seconds}초 안에 끝나지 않았습니다.",
        "fallback_engaged": "한컴 변환 엔진을 쓸 수 없어 rhwp로 진행합니다 — {detail}",
        ...
        "server_find": "찾기",
        "server_find_running": "네트워크에서 변환 서버를 찾는 중...",
        "server_find_none": "변환 서버를 찾지 못했습니다.\n\n주소를 직접 입력하거나, 서버 쪽 로그에 찍힌 초대 문자열을 주소창에 붙여 넣으세요.\n아래 버튼으로 이 컴퓨터가 붙어 있는 네트워크 대역 전체를 훑어볼 수도 있습니다 — 몇 초 걸리고, 회사망에서는 보안 장비가 포트 스캔으로 볼 수 있습니다.",
        "server_find_wide": "더 넓게 찾기",
        "server_find_wide_running": "네트워크 대역 전체를 훑는 중... (몇 초 걸립니다)",
        "server_find_wide_none": "대역 전체를 훑어도 응답한 서버가 없습니다.\n\n서버가 실행 중인지, 방화벽이 {port} 포트를 막고 있지 않은지 확인하세요. 서버가 다른 네트워크에 있다면 초대 문자열이 확실합니다.",
        "server_find_title": "변환 서버 선택",
        "server_find_hint": "Tailscale과 같은 네트워크에서 응답한 서버입니다. 목록에 없으면 서버 로그의 초대 문자열을 주소창에 붙여 넣으세요.",
        "server_find_select": "선택",
        "server_find_cancel": "취소",
        "server_find_column_name": "이름",
        "server_find_column_address": "주소",
        "server_find_column_version": "버전",
        "server_find_column_note": "비고",
        "server_find_via_tailscale": "Tailscale",
        "server_find_via_lan": "같은 네트워크",
        "server_find_needs_token": "토큰 필요",
        "server_find_incompatible": "버전 불일치",
        "server_invite_applied": "초대 문자열에서 주소와 토큰을 채웠습니다.",
        "server_address_invalid": "주소를 이해할 수 없습니다: {value}",
        ...
        "hwp_running_title": "실행 중인 아래한글 감지",
        "hwp_running_heading": "아래한글이 이미 실행 중입니다",
        "hwp_running_message": "감지됨: {process_detail}\n\n현재 문서를 보호하려면 rhwp로 PDF를 만들거나, 아래한글을 종료한 뒤 한컴 엔진으로 변환하세요.",
        "hwp_running_rhwp_note": "rhwp는 이번 작업에서 PDF만 근사 변환합니다. 결과가 원본과 다를 수 있습니다.",
        "hwp_running_rhwp_docx_note": "rhwp를 선택하면 PDF만 만들고 DOCX는 생성하지 않습니다. 결과가 원본과 다를 수 있습니다.",
        "hwp_action_rhwp": "rhwp로 PDF 변환",
        "hwp_action_kill": "아래한글 종료 후 변환",
        "hwp_action_continue": "그대로 한컴 사용",
        "hwp_action_cancel": "취소",
        ...
        "no_applicable_outputs": "선택한 입력에 적용할 출력 형식이 없습니다. HWPX 출력은 HWP 파일에만 적용됩니다.",
    ...
    "en": {
        ...
        "pick_file": "Pick file...",
        "options": "Options",
        "engine_status_title": "Conversion engine status",
        "hancom_install_checking": "Hancom Office Hangul: checking...",
        "hancom_install_ready": "Hancom Office Hangul: installed",
        "hancom_install_missing": "Hancom Office Hangul: not installed",
        "hancom_install_unsupported": "Local Hancom Office Hangul: unavailable on macOS",
        "hwp_status_running": "Hancom HWP process: running",
        "hwp_status_not_running": "Hancom HWP process: not running",
        ...
        "auto_update_confirm_macos": "Auto-update to v{latest}?\n\nThe update will be downloaded, this app replaced with the new version, and hwp2pdf reopened.",
        "auto_update_portable": "Auto-update is only supported for the installed build. Open the browser to download manually?",
        "auto_update_portable_macos": "This app sits somewhere it cannot replace itself. Move hwp2pdf.app somewhere writable, such as your Applications folder, and auto-update will work.\n\nOpen the browser to download manually?",
        ...
        "update_status_critical": "\u26a0 Important update available. Current: {current} / Latest: {latest}",
        "update_critical_prompt": "This update (v{latest}) is recommended immediately.\n\nUpdate now?",
        ...
        "select_output": "Select at least one output format: PDF, DOCX, or HWPX.",
        "pywin32_missing": "pywin32 is not available.\n\nInstall it with:\npython -m pip install pywin32\n\nDetails:\n{detail}",
        "local_hwp_missing": "Hancom Office Hangul was not found.\n\nInstall Hancom Office or check its COM registration.\n\nDetails: {detail}",
        "local_hwp_start_failed": "Could not start Hancom Office Hangul.\n\nDetails: {detail}",
        ...
        "rhwp_fallback_option": "Use rhwp for PDF when Hancom conversion is unavailable",
        "rhwp_fallback_help_local": "Used when Hancom Office is missing or cannot start · PDF only · output may differ from the original",
        "rhwp_fallback_help_remote": "Used when the conversion server is missing or unreachable · PDF only · output may differ from the original",
        "rhwp_status_ready": "rhwp installation: installed",
        "rhwp_status_missing": "rhwp installation: not installed",
        ...
        "rhwp_quality_note": "rhwp is an independent renderer, not Hancom's engine. Pagination, header page numbers and some glyphs can differ from the original, so re-convert final deliverables when Hancom conversion is available again.",
        "rhwp_format_unsupported": "rhwp can only produce PDF. {format} needs a Hancom conversion engine.",
        "rhwp_failed": "rhwp conversion failed: {detail}",
        "rhwp_timeout": "rhwp did not finish within {seconds}s.",
        "fallback_engaged": "Hancom conversion is unavailable, so rhwp is being used -- {detail}",
        ...
        "server_find": "Find",
        "server_find_running": "Looking for conversion servers on the network...",
        "server_find_none": "No conversion server answered.\n\nEnter an address directly, or paste the invite string from the server's log into the address field.\nThe button below sweeps every network this machine is attached to - it takes a few seconds, and on a corporate network a security appliance may read it as a port scan.",
        "server_find_wide": "Search wider",
        "server_find_wide_running": "Sweeping every attached network... (this takes a few seconds)",
        "server_find_wide_none": "Sweeping every attached network found nothing.\n\nCheck that the server is running and that no firewall blocks port {port}. If the server is on another network, the invite string is the reliable way in.",
        "server_find_title": "Choose a conversion server",
        "server_find_hint": "Servers that answered over Tailscale or on this network. If yours is not listed, paste the invite string from its log into the address field.",
        "server_find_select": "Select",
        "server_find_cancel": "Cancel",
        "server_find_column_name": "Name",
        "server_find_column_address": "Address",
        "server_find_column_version": "Version",
        "server_find_column_note": "Notes",
        "server_find_via_tailscale": "Tailscale",
        "server_find_via_lan": "This network",
        "server_find_needs_token": "token required",
        "server_find_incompatible": "API mismatch",
        "server_invite_applied": "Address and token filled in from the invite string.",
        "server_address_invalid": "Cannot make sense of the address: {value}",
        ...
        "hwp_running_title": "Running Hancom HWP detected",
        "hwp_running_heading": "Hancom HWP is already running",
        "hwp_running_message": "Detected: {process_detail}\n\nTo protect the open document, create an approximate PDF with rhwp or close HWP before converting with the Hancom engine.",
        "hwp_running_rhwp_note": "rhwp will create an approximate PDF for this run. The output may differ from the original.",
        "hwp_running_rhwp_docx_note": "If you choose rhwp, only PDF will be created; DOCX will not be created. The output may differ from the original.",
        "hwp_action_rhwp": "Convert PDF with rhwp",
        "hwp_action_kill": "Close HWP and convert",
        "hwp_action_continue": "Use Hancom as-is",
        "hwp_action_cancel": "Cancel",
        ...
        "no_applicable_outputs": "None of the selected output formats apply to this input. HWPX output applies to HWP files only.",
