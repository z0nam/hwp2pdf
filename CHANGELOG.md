...
- **배포된 맥 앱이 HTTPS를 전혀 쓰지 못하던 문제.** 파이썬의 `ssl`은 인터프리터를
  *빌드할 때* OpenSSL이 박아둔 경로에서 CA 인증서를 찾는데, PyInstaller로 얼려
  배포하면 그 경로가 사용자 맥에는 없습니다. 그래서 업데이트 확인도, **자동 업데이트
  다운로드도** `CERTIFICATE_VERIFY_FAILED`로 실패했습니다 — 즉 원클릭 업데이트가
  정작 배포판에서만 동작하지 않았습니다. `certifi`의 CA 번들을 앱에 같이 싣고,
  플랫폼 저장소가 비어 있을 때만 그쪽으로 넘어갑니다. Windows(시스템 저장소)와
  Linux(`/etc/ssl/certs`)는 하던 대로 자기 것을 씁니다 — 회사에서 사설 CA를 넣어둔
  경우가 계속 존중되어야 하기 때문입니다.
  로컬 빌드는 개발자 맥에 그 경로가 실제로 있어 통과하므로, 이 버그는 **CI 산출물을
  설치해야만** 재현됩니다.
...
- **긴급 릴리스 표시.** 릴리스 노트에 `hwp2pdf-priority: critical` 한 줄을 넣으면,
  앱이 상태줄에 조용히 버전만 알리는 대신 중요 업데이트임을 밝히고, 적용 전까지
  실행할 때마다 물어보며, 6시간이 아니라 1시간마다 다시 확인합니다.

### Changed
- 업데이트 확인 주기를 **24시간에서 6시간으로** 줄였습니다. 오후에 낸 릴리스가
  다음 날까지 보이지 않는 일이 실제로 있었습니다.
...
- 서버 주소를 몰라도 붙을 수 있는 경로를 추가했습니다.
  ...
  - **더 넓게 찾기**: 위 검색이 빈손일 때만 나타나는 버튼으로, 붙어 있는 사설
    대역의 `/24`를 훑습니다. 기본 경로뿐 아니라 모든 인터페이스의 대역을 보며
    (최대 3개), Tailscale 대역과 링크로컬은 제외합니다. 첫 클릭으로는 절대 돌지
    않습니다 — 회사망에서 포트 스캔으로 보일 수 있기 때문입니다.
    CLI는 `hwp2pdf find --wide`.
