...
### Fixed
- 이미 HWPX인 입력에 HWPX 출력을 요청해 원본 경로를 덮어쓸 수 있던 경우를 차단했습니다.
  HWPX 출력은 `.hwp` 입력에만 적용되며, HWPX 파일만 직접 선택하면 해당 체크박스가
  비활성화됩니다. 폴더·혼합 입력의 작업 수와 진행률도 실제 적용되는 변환만 셉니다.
...
- **맥 앱이 Finder·Alfred·자동 업데이트로 실행될 때 간헐적으로 죽던 문제.** 창이 뜨기
  ...
  자동 업데이트가 끝난 뒤 앱을 다시 여는 경로가 정확히 이 방식이라 업데이트 직후에
  특히 눈에 띄었습니다.
  원인은 Tk가 만드는 **Tcl 콘솔 창**입니다. Tk는 표준 입력이 "블록 0짜리 문자
  장치"일 때 그 창을 만드는데, launchd가 번들 앱에 주는 stdin이 바로 그것입니다
  (터미널에서 실행하면 이 경로를 타지 않아 멀쩡했습니다). 그 창의 메뉴바를 만들다
  macOS가 `NSMenuItem` 어서션으로 abort합니다 — 우리 코드가 한 줄도 실행되기 전입니다.
  이제 얼린 맥 빌드는 시작할 때 stdin을 쓰기 끝이 닫힌 파이프로 바꿉니다. `/dev/null`
  처럼 즉시 EOF지만 문자 장치가 아니라, 애초에 쓰지도 않던 콘솔 창이 만들어지지
  않습니다. 같은 소스로 10번 연속 정상 실행을 확인했습니다(적용 전 동일 빌드 5/8 사망).
  Tk 8.6·9.0 양쪽에서 재현되므로 Tk 버전 문제는 아닙니다.
...
- **변환 로그가 이제 실제로 보입니다.** 로그 위젯이 높이를 지정받지 않아 기본 24줄을
  ...
  변환이 실패했을 때 이유가 적히는 바로 그 자리인데요. 최소 6줄을 확보하고 기본 창
  높이를 그만큼(710 → 830) 올렸습니다. 창이 더 크면 로그도 함께 늘어납니다.
- **`변환 시작` 줄과 진행률 바를 대상 선택 바로 아래로 옮겼습니다.** 그동안은 옵션과
  변환 서버 패널(둘이 합쳐 442px) 뒤에 있었는데, Tk는 배치한 순서대로 공간을 나눠 주기
...
- **자동 업데이트가 실행 중인 변환 서버 때문에 중단되는 문제.** 설치본의 `hwp2pdf-serve.exe`도
  업데이트 대상인데 창 없는 서버는 조용한 설치 중 Restart Manager가 닫지 못해 설치가 취소될 수
  있었습니다. 이제 업데이트 도우미가 **같은 설치 경로의 서버만** 먼저 종료하고, 종료를 확인한
  뒤 설치합니다. 다른 위치에 설치된 서버는 건드리지 않습니다. 업데이트가 끝나면 서버는 다시
  시작해 주세요.
- **업데이트 실패 뒤 재실행할 때 PyInstaller 보안 검증 오류가 나는 문제.** 업데이트 도우미가
  상속한 일회용 실행 환경을 초기화한 뒤 앱을 다시 시작합니다.
- **파일을 넣으면 아래쪽 버튼이 사라지던 문제.** 사라진 게 아니라 창 밖으로 밀려난
  것이었습니다. 파일 목록이 나타나면 그만큼 아래가 밀리는데 창 크기는 그대로여서,
  Tk가 `변환 시작`·`중지` 줄을 아예 매핑에서 빼버렸습니다(서버 패널도 16px 잘림).
  이제 새 구역이 생기면 창이 그만큼 함께 커집니다. 커지기만 하고 줄지는 않으므로
  사용자가 키워 둔 창은 건드리지 않으며, 화면 높이를 넘지 않게 제한합니다.
